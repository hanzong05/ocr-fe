...
export function assembleFields(
  ...
  // Parse date_issued into day / month / year
  let ml_day = '', ml_month = '', ml_year = ''
  const raw = f.date_issued || ''
  const m1 = raw.match(/(\d{1,2})[-\/](\d{1,2})[-\/](\d{4})/)
  if (m1) { ml_month = m1[1]; ml_day = m1[2]; ml_year = m1[3] }
  else {
    const m2 = raw.match(/([A-Za-z]+)\s+(\d{1,2}),?\s+(\d{4})/)
    if (m2) { ml_month = m2[1]; ml_day = m2[2]; ml_year = m2[3] }
  }
  return {
    ...shared,
    ...
    groom_mother: join(f.groom_mother_name) || join(f.groom_mother_first, f.groom_mother_last),
    groom_mother_nat: f.groom_mother_citizenship || '',
    groom_father: join(f.groom_father_name) || join(f.groom_father_first, f.groom_father_last),
    ...
    bride_mother: join(f.bride_mother_name) || join(f.bride_mother_first, f.bride_mother_last),
    bride_mother_nat: f.bride_mother_citizenship || '',
    bride_father: join(f.bride_father_name) || join(f.bride_father_first, f.bride_father_last),

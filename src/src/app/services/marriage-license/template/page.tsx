...
export default function MarriageTemplatePage() {
  ...
  async function save() {
    setSaving(true)
    try {
      await saveDocument(Number(user?.id), 4, fields) // type_id 4 = MARRLIC
      ...
      <main style={{ flex: 1, padding: 24 }}>
        <div style={{ maxWidth: 820, margin: '0 auto' }}>
          <div className="no-print" style={{ display: 'flex', gap: 10, marginBottom: 20 }}>
            ...
            <button onClick={save} disabled={saving} style={{ padding: '10px 20px', background: 'var(--primary-green)', color: 'white', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 600, marginLeft: 'auto' }}>
              {saving ? 'Saving...' : 'SAVE'}
            </button>
          </div>

          <div style={{ background: 'white', borderRadius: 12, padding: '32px 40px', boxShadow: '0 2px 16px rgba(0,0,0,0.08)' }}>
            <Form90 fields={fields} editing={editing} onChange={(k, v) => setFields(p => ({ ...p, [k]: v }))} />
          </div>
        </div>
      </main>
    </div>

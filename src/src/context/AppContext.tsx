...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'SET_FILES':
      return { ...state, uploadedFiles: { ...state.uploadedFiles, [action.key]: action.files } }
    case 'SET_RECORDS':
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const setFiles = useCallback((key: 'cert' | 'marriage', files: File[]) =>
    dispatch({ type: 'SET_FILES', key, files }), [])
  const setRecords = useCallback((records: LcrRecord[]) =>
    dispatch({ type: 'SET_RECORDS', records }), [])

  return (
    <AppContext.Provider value={{ ...state, login, logout, notify, setFiles, setRecords }}>
      {children}
    </AppContext.Provider>
  )
}
